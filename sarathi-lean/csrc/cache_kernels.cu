...
void copy_blocks(
  ...
  SARATHI_DISPATCH_FLOATING_TYPES(
    key_caches[0].scalar_type(), "copy_blocks_kernel", ([&] {
      sarathi::copy_blocks_kernel<scalar_t><<<grid, block, 0, stream>>>(
  ...
  torch::Tensor& slot_mapping)  // [num_tokens]
{
  ...
  SARATHI_DISPATCH_FLOATING_TYPES(
    ...
    [&] {
      sarathi::reshape_and_cache_kernel<scalar_t><<<grid, block, 0, stream>>>(
  ...
  torch::Tensor& slot_mapping)  // [in]  [num_tokens]
{
  ...
  SARATHI_DISPATCH_FLOATING_TYPES(
    ...
    [&] {
      sarathi::gather_cached_kv_kernel_optimized<scalar_t><<<grid, block, 0, stream>>>(
        key.data_ptr<scalar_t>(),
        value.data_ptr<scalar_t>(),
        key_cache.data_ptr<scalar_t>(),
        value_cache.data_ptr<scalar_t>(),
  ...
  const std::string& kv_cache_dtype)
{
  ...
  SARATHI_DISPATCH_FLOATING_TYPES(
    ...
    [&] {
      sarathi::reshape_and_cache_flash_kernel<scalar_t><<<grid, block, 0, stream>>>(
        key.data_ptr<scalar_t>(),
        value.data_ptr<scalar_t>(),
...
template<typename scalar_t>
__global__ void cache_flat_kernel(
  const scalar_t* __restrict__ key,           // [num_tokens, num_heads, head_size]
  const scalar_t* __restrict__ value,         // [num_tokens, num_heads, head_size]
  scalar_t* __restrict__ k_cache,            // [num_blocks, block_size, num_heads, head_size]
  scalar_t* __restrict__ v_cache,            // [num_blocks, block_size, num_heads, head_size]
  ...
  const int key_stride,
  const int value_stride,
  const int k_cache_stride,
  const int v_cache_stride,
  const int num_heads,
  const int head_size) {
  // const int block_size) {
  const int64_t token_idx = blockIdx.x;
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    const int64_t src_key_idx = token_idx * key_stride + i;
    const int64_t src_value_idx = token_idx * value_stride + i;
    const int64_t key_cache_idx = token_idx * k_cache_stride + i;
    const int64_t value_cache_idx = token_idx * v_cache_stride + i;
  //   const int head_idx = i / head_size;
  //   const int head_offset = i % head_size;
  //   const int64_t tgt_value_idx = block_idx * block_stride
  //                             + block_offset * num_heads * head_size
  //                             + head_idx * head_size
  //                             + head_offset;
    k_cache[key_cache_idx] = __ldg(&key[src_key_idx]);
    v_cache[value_cache_idx] = __ldg(&value[src_value_idx]);
  }
}
} //namespace sarathi
  ...
  const std::string& kv_cache_dtype)
{
  ...
  // int block_stride = k_cache.stride(0);
  TORCH_CHECK(k_cache.stride(0) == v_cache.stride(0));
  int k_cache_stride = k_cache.stride(0);
  int v_cache_stride = v_cache.stride(0);
  ...
  SARATHI_DISPATCH_FLOATING_TYPES(
    ...
    "cache_flat",
    [&] {
      sarathi::cache_flat_kernel<scalar_t><<<grid, block, 0, stream>>>(
        key.data_ptr<scalar_t>(),
        value.data_ptr<scalar_t>(),
        k_cache.data_ptr<scalar_t>(),
        v_cache.data_ptr<scalar_t>(),
