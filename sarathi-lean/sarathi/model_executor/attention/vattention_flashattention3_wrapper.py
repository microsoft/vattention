...
# from flash_attn import fa3_flash_attn_func
...
class VAttentionFlashAttention3_Wrapper(BaseAttentionWrapper):
    _inst = None
    ...
    def forward(
        ...
        for prefill_cache_len_cpu, prefill_cache_len_device, query_len, current_len_device in zip(
            ...
            with self.get_timer(OperationMetrics.ATTN_INPUT_RESHAPE, layer_id):
                ...
                key_cache = kv_cache[0][index][:prefill_cache_len_cpu+query_len].reshape(1, -1, self.num_kv_heads, self.head_dim)
                value_cache = kv_cache[1][index][:prefill_cache_len_cpu+query_len].reshape(1, -1, self.num_kv_heads, self.head_dim)
            ...
            with self.get_timer(OperationMetrics.ATTN_KV_CACHE_SAVE, layer_id):
                ...
                # key_cache = key_cache[prefill_cache_len_cpu:prefill_cache_len_cpu+query_len]
                # value_cache = value_cache[prefill_cache_len_cpu:prefill_cache_len_cpu+query_len]
            ...
            with self.get_timer(OperationMetrics.ATTN_PREFILL, layer_id):
                ...
                #     )
                seq_output,_ = fa3_flash_attn_func(
                    seq_query.contiguous(),
