...
#define SYNC_GROUP \
    if constexpr(std::is_same_v<SyncGroup, void>) { \
        if constexpr(Kernel_traits::kNThreads == 32) \
            __syncwarp(); \
        else \
            __syncthreads(); \
    } else { \
...
__forceinline__ __device__ auto get_lse_tile(const Params &params, const int bidb, const int bidh, const int m_block, const flash::BlockInfo</*Varlen=*/!Is_even_MN> &binfo) {
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Return_softmax, typename Params, typename SyncGroup = void>
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block, const int tidx, char *smem_, SyncGroup *tg = nullptr) {
...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, 
    bool Is_softcap, bool Split, bool Append_KV, typename Params, typename SyncGroup = void>
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, 
    const int m_block, const int n_split_idx, const int num_n_splits, const int tidx, char *smem_, SyncGroup *tg = nullptr) {
...
inline __device__ void compute_attn(const Params &params) {
    ...
    if(tidx >= Kernel_traits::kNWarps * 32) return;
    ...
    fused::compute_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, 
        Is_softcap, Return_softmax>(params, bidb, bidh, m_block, tidx, smem_);
}
...
inline __device__ void compute_attn_splitkv(const Params &params) {
    ...
    // The thread index.
    const int tidx = threadIdx.x;
    if(tidx >= Kernel_traits::kNThreads) return;

    fused::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, 
        Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>
        (params, bidb, bidh, m_block, n_split_idx, num_n_splits, tidx, smem_);
}
...
template<typename Kernel_traits_prefill, typename Kernel_traits_decode, bool Is_dropout, bool Is_causal_p,  bool Is_causal_d,
    ...
    bool Is_softcap, bool Return_softmax, bool DecodeIsSplit, typename Params>
inline __device__ void compute_fused_attn(const Params &prefill_params, const Params &decode_params) {
    // We want the fwd and bwd to generate the same dropout pattern (RNG), without restricting
    // them to have the same number of threads or have to traverse the attention matrix
    // in the same order.
    // In the Philox RNG, we use the offset to store the batch, head, and the lane id
    ...
    // this CG group is problematic. Need both decodes and prefills to have same number of threads. 
    // Is there a better way?
    auto this_group = cg::tiled_partition<Kernel_traits_prefill::kNThreads>(cg::this_thread_block());
    //auto this_group = cg::this_thread_block();

    if(tidx < Kernel_traits_prefill::kNThreads) {
        //return;
        //dim3 grid(num_m_block, params.b, params.h);
        const int linear_block_id = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y;
        const int num_mblocks = (prefill_params.seqlen_q + Kernel_traits_prefill::kBlockM - 1) / Kernel_traits_prefill::kBlockM;
        if(linear_block_id >= num_mblocks * prefill_params.b * prefill_params.h) return;

        const int m_block = (linear_block_id % num_mblocks);
        // The block index for the batch.
        const int bidb = (linear_block_id / num_mblocks) % prefill_params.b;
        // The block index for the head.
        const int bidh = (linear_block_id / num_mblocks / prefill_params.b) % prefill_params.h;
        //const int m_block = blockIdx.x;
        //const int bidb = blockIdx.y;
        //const int bidh = blockIdx.z;
        ...
        fused::compute_attn_1rowblock<Kernel_traits_prefill, Is_dropout, Is_causal_p, 
            ...
            (prefill_params, bidb, bidh, m_block, tidx, smem_, &this_group);
    } else {
        if (DecodeIsSplit || decode_params.cache_batch_idx != nullptr) {
            //dim3 grid(num_m_block, params.num_splits, params.b * params.h);
            const int linear_block_id = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y;
            const int num_mblocks = (decode_params.seqlen_q + Kernel_traits_decode::kBlockM - 1) / Kernel_traits_decode::kBlockM;
            const int num_n_splits = decode_params.num_splits;
            if(linear_block_id >= num_mblocks * decode_params.b * decode_params.h * num_n_splits) return;
            
            const int m_block = linear_block_id % num_mblocks;
            const int n_split_idx = (linear_block_id / num_mblocks) % num_n_splits;
            const int bidb = (linear_block_id / num_mblocks / num_n_splits) % decode_params.b;
            const int bidh = (linear_block_id / num_mblocks / num_n_splits / decode_params.b) % decode_params.h;

            tidx -= Kernel_traits_prefill::kNThreads;
            char *smem_decode_ = smem_ + Kernel_traits_prefill::kSmemSize;
            fused::compute_attn_1rowblock_splitkv<Kernel_traits_decode, Is_causal_d, Is_local_d, 
                Has_alibi, Is_even_MN_d, Is_even_K_d, Is_softcap, DecodeIsSplit, false>
                (decode_params, bidb, bidh, m_block, n_split_idx, num_n_splits, tidx, smem_decode_, &this_group);
        } else {
            //dim3 grid(num_m_block, params.b, params.h);
            const int linear_block_id = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y;
            const int num_mblocks = (decode_params.seqlen_q + Kernel_traits_decode::kBlockM - 1) / Kernel_traits_decode::kBlockM;
            if(linear_block_id >= num_mblocks * decode_params.b * decode_params.h) return;

            const int m_block = linear_block_id % num_mblocks;
            // The block index for the batch.
            const int bidb = (linear_block_id / num_mblocks) % decode_params.b;
            // The block index for the head.
            const int bidh = linear_block_id / num_mblocks / decode_params.b;
            ...
            fused::compute_attn_1rowblock<Kernel_traits_decode, Is_dropout, Is_causal_d, 
                ...
                (decode_params, bidb, bidh, m_block, tidx, smem_decode_, &this_group);
        }
    }
}*/
...
template<typename Kernel_traits_prefill, typename Kernel_traits_decode, bool Is_dropout, bool Is_causal_p,  bool Is_causal_d,
    bool Is_local_p, bool Is_local_d, bool Has_alibi, bool Is_even_MN_p, bool Is_even_K_p, bool Is_even_MN_d, bool Is_even_K_d,
    bool Is_softcap, bool Return_softmax, bool DecodeIsSplit, bool DecodeAppend_KV, int FusedOp, typename Params>
inline __device__ void compute_fused_tb_attn(const Params &prefill_params, const Params &decode_params, int *tbAssign) {
    // We want the fwd and bwd to generate the same dropout pattern (RNG), without restricting
    // them to have the same number of threads or have to traverse the attention matrix
    // in the same order.
    // In the Philox RNG, we use the offset to store the batch, head, and the lane id
    ...
    // Dynamic threadblock scheduler code
    // Find out which SM this threadblock is scheduled on
    int num_SMs, sm_id;
    // WARNING: nsmid has only been tested on A100, and matches SM count (108)
    // No guarantee this will work on other GPUs
    asm volatile("mov.u32 %0, %nsmid;" : "=r"(num_SMs));
    asm volatile("mov.u32 %0, %smid;" : "=r"(sm_id));

    const int prefill_blocks = (num_mblocks_p * prefill_params.b * prefill_params.h);
    ...
    if(threadIdx.x == 0) {
        ...
        if constexpr(FusedOp & 1) {
            const int min_blocks = min(prefill_slots, decode_slots);
            const int total_tags = (prefill_slots + decode_slots) / min_blocks;
            
            // For this SM, what's the next operation we want to run?
            int tag = (atomicAdd(&tbAssign[sm_id], 1) % total_tags);
            if(tag < prefill_slots / min_blocks) {
                op = 0;
            } else {
                op = 1;
            }
        } else {
            op = atomicAdd(&tbAssign[sm_id], 1) % 2;
        }

        // Get the next blockId for that operation
        int bid = atomicAdd(&tbAssign[num_SMs + op], 1);
        // If the blockId obtained exceeds the max blockIds for that op, switch to the other op
        if(op == 0 && bid >= prefill_slots) {
            op = (op + 1) % 2;
            bid = atomicAdd(&tbAssign[num_SMs + op], 1);
        } else if (op == 1 && bid >= decode_slots) {
            op = (op + 1) % 2;
            bid = atomicAdd(&tbAssign[num_SMs + op], 1);
        }
    ...
    int linear_block_id = ((int*)smem_)[0];
    const int op = ((int*)smem_)[1];

    if(op == 0) {
        // Remap to logical threadblock from real threadblock
        linear_block_id = linear_block_id * blk_factor_p + tidx / Kernel_traits_prefill::kNThreads;
        char *smem_map = smem_ + tidx / Kernel_traits_prefill::kNThreads * Kernel_traits_prefill::kSmemSize;
        ...
        // Remap to logical threadblock from real threadblock
        linear_block_id = linear_block_id * blk_factor_d + tidx / Kernel_traits_decode::kNThreads;
        char *smem_map = smem_ + tidx / Kernel_traits_decode::kNThreads * Kernel_traits_decode::kSmemSize;
        tidx = tidx % Kernel_traits_decode::kNThreads;
        if(linear_block_id >= decode_blocks) return;

        if (DecodeIsSplit || DecodeAppend_KV || decode_params.cache_batch_idx != nullptr) {
            //dim3 grid(num_m_block, params.num_splits, params.b * params.h);
            const int num_n_splits = decode_params.num_splits;
