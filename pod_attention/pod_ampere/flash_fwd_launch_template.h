...
template<typename Kernel_traits, bool Is_causal, bool Split>
...
void run_mha_fwd_splitkv_dispatch(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if(params.num_splits <= 1) {
        if(params.fused_params == 0)
            run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 64, 128, 4, false, false, T>, Is_causal, false>(params, stream);
        else if(params.fused_params == 2)
            run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 32, 64, 2, false, false, T>, Is_causal, false>(params, stream);
        else if(params.fused_params == 3)
            run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 16, 32, 1, false, false, T>, Is_causal, false>(params, stream);
        else
            run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, kBlockM, kBlockN, 4, false, false, T>, Is_causal, false>(params, stream);
    } else {
        run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, kBlockM, kBlockN, 4, false, false, T>, Is_causal, true>(params, stream);
    }
}
    ...
    //DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        if constexpr(!Is_dropout) {
            ...
            if constexpr(false && is_sm8x) {
                ...
                // Fix tile size for highly parallel decodes
                if(params.seqlen_q <= 64 && params.fused_params == 0)
                    run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 64, 128, 4, false, false, T>, Is_dropout, Is_causal>(params, stream);
                else if(params.fused_params == 2)
                    run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 32, 64, 2, false, false, T>, Is_dropout, Is_causal>(params, stream);
                else if(params.fused_params == 3)
